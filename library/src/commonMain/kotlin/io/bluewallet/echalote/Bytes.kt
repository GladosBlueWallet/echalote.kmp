...
fun hexToBytes(hex: String): ByteArray {
    val s = hex.trim()
    require(s.length % 2 == 0) { "odd hex length: ${s.length}" }
...
internal fun ByteArray.u32be(i: Int): Int = (u8(i) shl 24) or (u8(i + 1) shl 16) or (u8(i + 2) shl 8) or u8(i + 3)

internal fun ByteArray.putU8(
    i: Int,
    v: Int,
) {
    this[i] = v.toByte()
...
internal fun ByteArray.putU16be(
    i: Int,
    v: Int,
) {
...
internal fun ByteArray.putU32be(
    i: Int,
    v: Int,
) {
...
internal fun ByteArray.putU64be(
    i: Int,
    v: Long,
) {
    for (b in 0 until 8) {
