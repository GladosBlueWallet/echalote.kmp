...
class BuildExitCircuitTest {
    ...
    @Test
    fun retriesTransientBuildOnceFailuresUpToAttempts() =
        runTest {
            var calls = 0
    ...
    @Test
    fun doesNotRetryNonTransientErrors() =
        runTest {
            var calls = 0
            val client = TorClientDuplex()
            val ex =
    ...
    @Test
    fun racesParallelBuildsAndClosesLosers() =
        runTest {
            val client = TorClientDuplex()
    ...
    @Test
    fun rejectsNonPositiveCircuitRace() =
        runTest {
            val client = TorClientDuplex()
            val ex =
                assertFails {
...
class RaceFirstCircuitTest {
    @Test
    fun rejectsWhenParentAbortsEvenIfBuilderIgnoresSignal() =
        runTest {
            val parent = Abort()
            supervisorScope {
                val pending =
    ...
    @Test
    fun doesNotAbortTheWinnerSignalAfterSettle() =
        runTest {
