...
class ClearnetParallelTest {
    @Test
    fun returnsFirstSuccessfulResultAndCancelsSlowerWork() =
        runTest {
    ...
    @Test
    fun triesUntilOneSucceedsWithinConcurrency() =
        runTest {
            var n = 0
            val result =
                fetchFirstOk(
    ...
    @Test
    fun rejectsAggregateErrorWhenEveryWorkerFails() =
        runTest {
            val ex =
                assertFails {
                    fetchFirstOk(
    ...
    @Test
    fun rejectsOnParentAbortEvenIfWorkerIgnoresSignal() =
        runTest {
            val parent = Abort()
            supervisorScope {
                val pending =
